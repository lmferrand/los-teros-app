  ...
  return (
    <div className="min-h-screen bg-gray-950">
      <div className="bg-gray-900 border-b border-gray-800 px-6 py-4 flex items-center gap-4">
        ...
        {perfil && (
          <span className="text-gray-400 text-sm ml-auto">{perfil.nombre}</span>
        )}
      </div>
        ...
        {orden && (
          <div className="bg-blue-950 border border-blue-800 rounded-xl p-4 mb-6">
            ...
            <p className="text-blue-300 text-xs mt-1">
              Los materiales se vincularan automaticamente a esta OT
            </p>
          </div>
        )}
          ...
          <div className="text-center">
            <div className="bg-gray-900 border border-gray-800 rounded-xl p-8 mb-6">
              ...
              <p className="text-gray-400 text-sm mb-6">
                Apunta la camara al codigo QR del material o equipo para registrar la salida
              </p>
              <button
                onClick={iniciarScanner}
                className="bg-blue-600 hover:bg-blue-700 text-white px-8 py-3 rounded-xl text-sm font-medium w-full"
              >
                Abrir camara
              </button>
            </div>
            ...
            {mensaje && (
              <div className={`rounded-xl p-4 text-sm ${mensaje.startsWith('OK') ? 'bg-green-950 border border-green-800 text-green-300' : 'bg-red-950 border border-red-800 text-red-300'}`}>
                ...
                <div className="flex gap-2 justify-center flex-wrap">
                  <button
                    onClick={() => { setMensaje(''); iniciarScanner() }}
                    className="bg-gray-800 hover:bg-gray-700 text-white px-4 py-2 rounded-lg text-xs"
                  >
                    Escanear otro
                  </button>
                  {ordenId && (
        ...
        {escaneando && (
          <div>
            ...
            <button
              ...
              className="w-full bg-gray-800 hover:bg-gray-700 text-gray-300 px-4 py-3 rounded-xl text-sm"
            >
              Cancelar
            </button>
          </div>
          ...
          <div className="bg-gray-900 border border-gray-800 rounded-xl p-6">
              ...
              <div>
                ...
                {item._tipo === 'material' && (
                  <p className="text-gray-400 text-sm">
                    Stock disponible: <span className={`font-bold ${(item.stock || 0) <= (item.minimo || 0) ? 'text-red-400' : 'text-green-400'}`}>{item.stock || 0} {item.unidad || ''}</span>
                  </p>
                )}
                {item._tipo === 'equipo' && (
                  <p className="text-gray-400 text-sm">
                    {item.tipo} — Estado: <span className="text-white">{item.estado}</span>
                  </p>
                )}
              </div>
            </div>
            ...
            {item._tipo === 'material' && (
              <div className="mb-5">
                ...
                <div className="flex items-center gap-3">
                  <button
                    onClick={() => setCantidad(prev => String(Math.max(0.5, parseFloat(prev) - 0.5)))}
                    className="w-12 h-12 bg-gray-800 hover:bg-gray-700 text-white rounded-xl text-xl font-bold"
                  >
                    -
                  </button>
                  <input
                    ...
                    step="0.5"
                    max={item.stock || 0}
                    className="flex-1 bg-gray-800 border border-gray-700 rounded-xl px-3 py-3 text-white text-2xl text-center font-bold"
                  />
                  <button
                    onClick={() => setCantidad(prev => String(Math.min(item.stock || 0, parseFloat(prev) + 0.5)))}
                    className="w-12 h-12 bg-gray-800 hover:bg-gray-700 text-white rounded-xl text-xl font-bold"
                  >
                    +
                  </button>
            ...
            {item._tipo === 'equipo' && item.estado !== 'disponible' && (
              <div className="bg-red-950 border border-red-800 rounded-lg p-3 mb-4">
                <p className="text-red-300 text-sm">
                  Este equipo no esta disponible. Estado actual: {item.estado.replace('_', ' ')}
                </p>
              </div>
            )}
            ...
            <div className="flex gap-3">
              <button
                ...
                className="flex-1 bg-blue-600 hover:bg-blue-700 disabled:opacity-50 disabled:cursor-not-allowed text-white px-4 py-3 rounded-xl text-sm font-medium"
              >
                {guardando ? 'Registrando...' : item._tipo === 'material' ? 'Confirmar salida' : 'Registrar equipo'}
              </button>
              <button
                onClick={() => { setItem(null); setMensaje(''); setCantidad('1') }}
                className="bg-gray-800 hover:bg-gray-700 text-gray-300 px-4 py-3 rounded-xl text-sm"
              >
                Cancelar
              </button>
            </div>
