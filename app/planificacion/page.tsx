    ...
    <div className="min-h-screen" style={{ background: '#080b14' }}>
      <div className="px-6 py-4 flex items-center justify-between flex-wrap gap-3" style={{ background: '#0d1117', borderBottom: '1px solid #1e2d3d' }}>
        <div className="flex items-center gap-4">
          ...
          <h1 className="text-white font-bold text-lg">Planificacion</h1>
        </div>
        {vistaActiva === 'calendario' && (
          <div className="flex items-center gap-3">
            <button onClick={mesAnterior} className="text-sm px-3 py-2 rounded-xl transition-colors"
              style={{ background: '#080b14', color: '#64748b', border: '1px solid #1e2d3d' }}>Anterior</button>
            <span className="text-white font-mono font-bold text-sm min-w-40 text-center">{tituloMes}</span>
            <button onClick={mesSiguiente} className="text-sm px-3 py-2 rounded-xl transition-colors"
              style={{ background: '#080b14', color: '#64748b', border: '1px solid #1e2d3d' }}>Siguiente</button>
          </div>
        )}
        {vistaActiva === 'presupuestos' && (
          <button onClick={() => abrirFormPres()} className="text-white text-sm px-4 py-2 rounded-xl font-medium"
      ...
      <div className="p-6 max-w-6xl mx-auto">
        ...
        {ordenSeleccionada && (
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4" style={{ background: 'rgba(0,0,0,0.85)' }}>
            <div className="w-full max-w-lg max-h-screen overflow-y-auto rounded-2xl" style={cardStyle}>
              <div className="sticky top-0 px-6 py-4 flex items-start justify-between rounded-t-2xl" style={{ background: '#0d1117', borderBottom: '1px solid #1e2d3d' }}>
                <div>
              ...
              <div className="p-6 space-y-3 text-sm">
                {[
                  ...
                  { label: 'Duracion', val: <span className="text-white">{ordenSeleccionada.duracion_horas || 2}h</span> },
                  { label: 'Hora fija', val: <span className="text-white">{ordenSeleccionada.hora_fija ? 'Si' : 'No'}</span> },
                  { label: 'Fecha', val: <span className="text-white text-xs">{new Date(ordenSeleccionada.fecha_programada).toLocaleDateString('es-ES', { day: '2-digit', month: '2-digit', year: 'numeric', hour: '2-digit', minute: '2-digit' })}</span> },
                  { label: 'Trabajadores', val: <span className="text-white text-xs">{getNombresTecnicos(ordenSeleccionada.tecnicos_ids || [])}</span> },
                ].map((item, i) => (
                ...
                {ordenSeleccionada.descripcion && (
                  <div>
                    <p className="mb-1" style={{ color: '#475569' }}>Trabajos</p>
                    <p className="text-white rounded-xl p-3 text-xs leading-relaxed" style={{ background: '#080b14', border: '1px solid #1e2d3d' }}>{ordenSeleccionada.descripcion}</p>
                  </div>
                )}
                {ordenSeleccionada.observaciones && (
                  <div>
                    <p className="mb-1" style={{ color: '#475569' }}>Observaciones</p>
                    <p className="text-white rounded-xl p-3 text-xs leading-relaxed" style={{ background: '#080b14', border: '1px solid #1e2d3d' }}>{ordenSeleccionada.observaciones}</p>
                  </div>
                )}
                <div className="flex gap-3 pt-2">
                  <button onClick={() => { router.push('/ordenes'); setOrdenSeleccionada(null) }}
                    ...
                    style={{ background: 'linear-gradient(135deg, #7c3aed, #06b6d4)' }}>Ver en OT</button>
                  <button onClick={() => setOrdenSeleccionada(null)}
                    className="text-sm px-4 py-2 rounded-xl"
                    style={{ background: '#080b14', color: '#64748b', border: '1px solid #1e2d3d' }}>Cerrar</button>
                </div>
              </div>
            </div>
          </div>
        ...
        {vistaActiva === 'calendario' && (
          <>
            <div className="rounded-2xl overflow-hidden mb-6" style={cardStyle}>
              <div className="grid grid-cols-7" style={{ borderBottom: '1px solid #1e2d3d' }}>
                {['Lun', 'Mar', 'Mie', 'Jue', 'Vie', 'Sab', 'Dom'].map(d => (
                  <div key={d} className="text-center py-2 text-xs font-bold uppercase tracking-wider" style={{ color: '#334155' }}>{d}</div>
                ))}
              </div>
              <div className="grid grid-cols-7">
                {dias.map((dia, i) => {
                  if (!dia) return <div key={i} className="min-h-24" style={{ borderBottom: '1px solid #1e2d3d', borderRight: '1px solid #1e2d3d', background: '#080b14', opacity: 0.3 }} />
                  ...
                  return (
                    <div key={i} className="min-h-24 p-1.5" style={{ borderBottom: '1px solid #1e2d3d', borderRight: '1px solid #1e2d3d', background: esHoy ? 'rgba(124,58,237,0.08)' : 'transparent' }}>
                      <p className="text-xs font-bold mb-1" style={{ color: esHoy ? '#a78bfa' : '#334155' }}>{dia.getDate()}</p>
                      {otsDelDia.map(o => (
                        <button key={o.id} onClick={() => setOrdenSeleccionada(o)}
                          className="w-full text-left text-xs px-1.5 py-1 rounded-lg mb-1 truncate transition-opacity hover:opacity-80"
                          style={{ background: COLORES_OT[o.tipo]?.bg || 'rgba(71,85,105,0.2)', color: COLORES_OT[o.tipo]?.color || '#64748b' }}>
                          {o.codigo}
                        </button>
                      ))}
                    </div>
            ...
            <div className="rounded-2xl p-5" style={cardStyle}>
              ...
              {otsSemana.length === 0 ? (
                <p className="text-sm" style={{ color: '#475569' }}>Sin trabajos programados esta semana</p>
              ) : (
                  ...
                  {otsSemana.map(o => (
                    <div key={o.id} onClick={() => setOrdenSeleccionada(o)}
                      ...
                      <div>
                        ...
                        <p className="text-xs mt-1" style={{ color: '#475569' }}>{(o.descripcion || '').substring(0, 80)}</p>
                      </div>
                      <div className="text-right ml-4">
                        <p className="text-xs" style={{ color: '#475569' }}>{new Date(o.fecha_programada).toLocaleDateString('es-ES', { weekday: 'short', day: '2-digit', month: '2-digit' })}</p>
                        <p className="text-xs" style={{ color: '#334155' }}>{new Date(o.fecha_programada).toLocaleTimeString('es-ES', { hour: '2-digit', minute: '2-digit' })}</p>
                      </div>
                    </div>
                  ))}
                </div>
        ...
        {vistaActiva === 'mis_ordenes' && (
          <div>
            <div className="mb-6">
              ...
              <p className="text-sm mb-4" style={{ color: '#475569' }}>Solo las ordenes asignadas a ti</p>
              {misOrdenesPendientes.length === 0 ? (
                <div className="text-center py-12 rounded-2xl" style={cardStyle}>
                  <p className="text-3xl mb-2">✅</p>
                  <p style={{ color: '#475569' }}>No tienes ordenes pendientes</p>
                </div>
              ) : (
                <div className="flex flex-col gap-3">
                    ...
                    <div key={o.id} onClick={() => setOrdenSeleccionada(o)}
                      ...
                      <div className="flex items-start justify-between flex-wrap gap-3">
                        <div>
                          ...
                          <p className="text-sm mt-1" style={{ color: '#475569' }}>{(o.descripcion || '').substring(0, 120)}</p>
                        ...
                        <div className="text-right">
                          ...
                          <p className="text-xs" style={{ color: '#475569' }}>{o.fecha_programada ? new Date(o.fecha_programada).toLocaleTimeString('es-ES', { hour: '2-digit', minute: '2-digit' }) : ''}</p>
                        </div>
                      </div>
                    </div>
                  ))}
              ...
              <div>
                ...
                <div className="flex flex-col gap-2">
                  {misOrdenesCompletadas.map(o => (
                    <div key={o.id} onClick={() => setOrdenSeleccionada(o)}
                      className="rounded-xl p-4 cursor-pointer transition-all opacity-60" style={cardStyle}
        ...
        {vistaActiva === 'presupuestos' && (
          <div>
            ...
            <div className="rounded-2xl p-5 mb-6" style={cardStyle}>
              ...
              <p className="text-sm mb-4" style={{ color: '#475569' }}>Fotografía o sube el presupuesto de Holded y los datos se rellenaran automaticamente.</p>
              <label className="w-full flex items-center justify-center gap-3 px-4 py-3 rounded-xl text-sm font-medium cursor-pointer transition-all"
                style={escaneando ? { background: '#080b14', color: '#475569', border: '1px solid #1e2d3d' } : { background: 'linear-gradient(135deg, #7c3aed, #06b6d4)', color: 'white' }}>
            ...
            {mostrarFormPres && (
              <div className="rounded-2xl p-6 mb-6" style={cardStyle}>
                <h2 className="text-white font-semibold mb-5">{editandoPres ? 'Editar presupuesto' : 'Nuevo presupuesto'}</h2>
                <form onSubmit={guardarPresupuesto} className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div>
                    ...
                    <select value={presClienteId} onChange={e => setPresClienteId(e.target.value)} className="w-full rounded-xl px-3 py-2 text-sm outline-none" style={inputStyle}>
                  ...
                  <div>
                    ...
                    <input value={presTitulo} onChange={e => setPresTitulo(e.target.value)} required className="w-full rounded-xl px-3 py-2 text-white text-sm outline-none" style={inputStyle} placeholder="Limpieza campanas..." />
                  </div>
                  <div>
                    <label className="text-xs uppercase tracking-wider mb-2 block" style={{ color: '#475569' }}>Importe (EUR)</label>
                    <input type="number" value={presImporte} onChange={e => setPresImporte(e.target.value)} className="w-full rounded-xl px-3 py-2 text-white text-sm outline-none" style={inputStyle} min="0" step="0.01" />
                  </div>
                  <div>
                    <label className="text-xs uppercase tracking-wider mb-2 block" style={{ color: '#475569' }}>Fecha envio</label>
                    <input type="date" value={presFecha} onChange={e => setPresFecha(e.target.value)} className="w-full rounded-xl px-3 py-2 text-white text-sm outline-none" style={inputStyle} />
                  </div>
                  <div>
                    <label className="text-xs uppercase tracking-wider mb-2 block" style={{ color: '#475569' }}>Estado</label>
                    <select value={presEstado} onChange={e => setPresEstado(e.target.value)} className="w-full rounded-xl px-3 py-2 text-sm outline-none" style={inputStyle}>
                  ...
                  <div>
                    ...
                    <input value={presObs} onChange={e => setPresObs(e.target.value)} className="w-full rounded-xl px-3 py-2 text-white text-sm outline-none" style={inputStyle} placeholder="Notas..." />
                  </div>
            ...
            {presupuestos.length === 0 ? (
              <div className="text-center py-16 rounded-2xl" style={cardStyle}>
                <p className="text-3xl mb-2">📄</p>
                <p style={{ color: '#475569' }}>No hay presupuestos. Crea el primero o escanea uno.</p>
              </div>
            ) : (
              <div className="flex flex-col gap-3">
                {presupuestos.map(p => (
                  <div key={p.id} className="rounded-2xl p-5 transition-all" style={cardStyle}
                    onMouseEnter={e => e.currentTarget.style.borderColor = '#7c3aed'}
                    onMouseLeave={e => e.currentTarget.style.borderColor = '#1e2d3d'}>
                    <div className="flex items-start justify-between flex-wrap gap-3 mb-3">
                      <div>
                        ...
                        <p className="text-sm" style={{ color: '#64748b' }}>{p.clientes?.nombre || '—'}</p>
                        {(() => {
                          const cli = clientes.find(c => c.id === p.cliente_id)
                          return cli ? (
                            <div className="flex gap-4 mt-2 flex-wrap">
                              {cli.telefono && (
                                <a href={`tel:${cli.telefono}`} className="flex items-center gap-1 text-xs font-medium transition-colors" style={{ color: '#34d399' }}>
                                  📞 {cli.telefono}
                                </a>
                              )}
                              {cli.email && (
                                <a href={`mailto:${cli.email}`} className="flex items-center gap-1 text-xs font-medium transition-colors" style={{ color: '#06b6d4' }}>
                                  ✉️ {cli.email}
                                </a>
                              )}
                            </div>
                        ...
                        {p.observaciones && <p className="text-xs mt-1" style={{ color: '#475569' }}>{p.observaciones}</p>}
                      </div>
                      <div className="text-right">
                        <p className="font-bold font-mono text-lg text-white">{(p.importe || 0).toFixed(2)} EUR</p>
                        <p className="text-xs" style={{ color: '#475569' }}>{p.fecha_envio ? new Date(p.fecha_envio).toLocaleDateString('es-ES') : '—'}</p>
                      </div>
                    </div>
                    <div className="flex flex-wrap gap-2 items-center pt-3" style={{ borderTop: '1px solid #1e2d3d' }}>
                      <p className="text-xs mr-1" style={{ color: '#334155' }}>Estado:</p>
        ...
        {vistaActiva === 'rutas' && (
          <div>
            <div className="rounded-2xl p-5 mb-6" style={cardStyle}>
              ...
              <p className="text-sm mb-4" style={{ color: '#475569' }}>Selecciona un dia y el sistema calculara la ruta optima partiendo desde Elche.</p>
              <div className="flex gap-3 items-end flex-wrap">
                <div>
                  <label className="text-xs uppercase tracking-wider mb-2 block" style={{ color: '#475569' }}>Dia a planificar</label>
                  <input type="date" value={fechaRuta} onChange={e => setFechaRuta(e.target.value)}
                    className="rounded-xl px-3 py-2 text-white text-sm outline-none" style={inputStyle} />
                </div>
                <button onClick={calcularRutas} disabled={calculando}
                  className="text-white px-6 py-2 rounded-xl text-sm font-medium disabled:opacity-50"
                  style={{ background: 'linear-gradient(135deg, #7c3aed, #06b6d4)' }}>
            ...
            {resultadoRuta?.vacio && (
              <div className="text-center py-12 rounded-2xl" style={cardStyle}>
                <p className="text-3xl mb-2">📅</p>
                <p style={{ color: '#475569' }}>No hay ordenes pendientes para ese dia.</p>
              </div>
            )}
            ...
            {resultadoRuta?.resultados && (
              <div className="flex flex-col gap-6">
                <div className="rounded-2xl p-4" style={cardStyle}>
                  ...
                  <p className="text-sm mt-1" style={{ color: '#475569' }}>{resultadoRuta.totalOTs} ordenes — {resultadoRuta.resultados.length} trabajadores</p>
                </div>

                {resultadoRuta.resultados.map((res: any) => (
                  <div key={res.tecnico.id} className="rounded-2xl overflow-hidden" style={cardStyle}>
                    <div className="px-5 py-3 flex items-center justify-between flex-wrap gap-2" style={{ background: '#080b14', borderBottom: '1px solid #1e2d3d' }}>
                      <div>
                        <p className="text-white font-semibold">{res.tecnico.nombre}</p>
                        <p className="text-xs" style={{ color: '#475569' }}>Salida 08:00 desde Elche — {res.ruta.length} paradas — {res.horasTotales}h</p>
                      </div>
                      ...
                      <div className="flex flex-col gap-2">
                        <div className="flex items-center gap-3">
                          <div className="w-8 h-8 rounded-xl flex items-center justify-center text-xs font-bold flex-shrink-0"
                            style={{ background: 'rgba(71,85,105,0.3)', color: '#64748b' }}>S</div>
                          <div className="flex-1 rounded-xl px-3 py-2 flex items-center justify-between"
                            ...
                            <div>
                              ...
                              <p className="text-xs" style={{ color: '#475569' }}>C/ Leonardo Da Vinci 12</p>
                            </div>
                          ...
                          <div key={parada.ot.id}>
                            {parada.traslado > 0 && (
                              <div className="flex items-center gap-3 my-1 pl-4">
                                <div className="w-0.5 h-5 ml-3.5" style={{ background: '#1e2d3d' }}></div>
                                <p className="text-xs" style={{ color: '#334155' }}>Traslado ~{parada.traslado} min{ZONAS[parada.zona] ? ` — ${ZONAS[parada.zona].nombre}` : ''}</p>
                              </div>
                            )}
                            <div className="flex items-start gap-3">
                              ...
                              <div className="flex-1 rounded-xl px-3 py-3 transition-colors"
                                style={parada.horaFija
                                  ...
                                  : { background: '#080b14', border: '1px solid #1e2d3d' }}>
                                <div className="flex items-start justify-between flex-wrap gap-2">
                                  <div>
                                    ...
                                    <p className="text-xs" style={{ color: '#475569' }}>{parada.cliente?.direccion || '—'}</p>
                                    <p className="text-xs mt-1 capitalize" style={{ color: '#334155' }}>{parada.ot.tipo} — {parada.ot.duracion_horas || 2}h</p>
                                  </div>
                                  <div className="text-right">
                                    <p className="text-white text-sm font-mono font-bold">{formatHora(parada.horaInicio)}</p>
                                    <p className="text-xs" style={{ color: '#475569' }}>hasta {formatHora(parada.horaFin)}</p>
                                    {parada.cliente?.direccion && (
                                      <a href={`https://www.google.com/maps/dir/Calle+Leonardo+Da+Vinci+12+Elche/${encodeURIComponent(parada.cliente.direccion)}`}
                                        target="_blank" rel="noreferrer" className="text-xs block mt-1 transition-colors" style={{ color: '#06b6d4' }}>
                                        Abrir Maps
                                      </a>
                                    )}
                                  </div>
                        ...
                        <div className="flex items-center gap-3 mt-2">
                          <div className="w-8 h-8 rounded-xl flex items-center justify-center text-xs font-bold flex-shrink-0"
                            style={{ background: 'rgba(71,85,105,0.3)', color: '#64748b' }}>F</div>
                          <div className="flex-1 rounded-xl px-3 py-2 flex items-center justify-between"
                            style={{ background: '#080b14', border: '1px solid #1e2d3d' }}>
